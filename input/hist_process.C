...
void processsimulation(std::pair<TString, Double_t> sample, TString filename, std::map<TString, Int_t> precut, std::map<TString, Int_t> cut, std::map<TString, Int_t> particletype, TLorentzVector CME, Double_t luminosity, TString directory){
    ...
    if (luminosity != 0.0) { // Checks if sample exists
...
void hist_process(){
    ...
    TString filename = "madgraph"; // Filename for created histogram collection
    TString directory = "/afs/cern.ch/work/a/aandriat/public/test/MIT-FCCee/delphes/"; // Directory of stored delphes samples
    ...
    { 1, std::pair<TString, Double_t> ("zh_test", 26.12)},
    { 2, std::pair<TString, Double_t> ("ww_test", 30.69)}
    };
