...
int main() {
  ...
  //Random:setSeed = on;
  //Random:seed = (int) (time(0) - 1400000000);

  // H to ZZ to vvvv for ZH sample
  // pythia.readString("25:onMode = off");
  // pythia.readString("25:onIfMatch = 23 23"); //H->ZZ only
  // pythia.readString("23:onMode = off");
  // pythia.readString("23:onIfAny = 12 14 16"); //Z->vv (invisible so wont be detected) 

  // Z->l+l- for ZZ and WW samples
  pythia.readString("23:onMode = off"); 
  pythia.readString("23:onIfMatch = 11 11");
  pythia.readString("23:onIfMatch = 13 13");
  pythia.readString("23:onIfMatch = 15 15");

  pythia.readString("24:onMode = off"); //W->l,nu only
  pythia.readString("24:onIfAny = 11 13 15");
