...
Int_t FSR_count = 0;
template<typename T>
TString parameter_functions(TClonesArray *branch, T (*examplePart), TString parttype, std::map<TString, TClonesArray*> branches, std::map<TString, Int_t> cut, TLorentzVector CME, TTree *ttree, std::map<TString, Float_t> *parameters, Float_t weight, Float_t numUnweightedEntries){
    ...
    TLorentzVector sumallP4, diffallP4, particle1P4, particle2P4, photonP4, sumparticleP4, diffparticleP4, 
                   trackP4, towerP4, scaledparticle1P4, scaledparticle2P4, scaledsumP4, scaleddiffP4;
    ...
    Float_t recoilmass = 0, vism = 0, vise = 0, vispt = 0, vispz = 0, ivise = 0, ivispt = 0, ivispz = 0, 
            ...
            labangle = 0, btag = 0, tautag = 0, numtracks = 0, numjets = 0, costheta1 = 0, costheta2 = 0, eta1 = 0, 
            ...
            scaledrecoilmass = 0, scaleddivism = 0;
    ...
    if (parttype == "jet") {
        chosen = select_2_leading(branch, particle); 
    }
    ...
    //Scaling Test!
    scaledparticle1P4 = particle1P4*(MASS_Z/sumparticleP4.M());
    scaledparticle2P4 = particle2P4*(MASS_Z/sumparticleP4.M());
    scaledsumP4 = scaledparticle1P4 + scaledparticle2P4;
    scaleddiffP4 = CME-scaledsumP4;

    scaledrecoilmass = scaleddiffP4.M();
    (*parameters)["scaledrecoilmass"] = scaledrecoilmass;
    scaleddivism = scaledsumP4.M();
    (*parameters)["scaleddivism"] = scaleddivism;
    //
    ...
    for (Int_t iObject = 0; iObject < branches["EFlowNeutralHadron"]->GetEntries(); iObject++){ // Sum of visible mass - used for cuts
        eflowneutralhadron = (Tower*) branches["EFlowNeutralHadron"]->At(iObject);
    ...
    */
    ...
    //2 Jet collections
    //
    Jet *je = 0, *jet2Sel1 = 0, *jet2Sel2 = 0;
    TLorentzVector jet1P4, jet2P4, sumjet2P4, diffjet2P4, scaledjet1P4, scaledjet2P4, scaledsum2P4, scaleddiff2P4;
    Float_t INCdivism = 0, INCdivispt = 0, INCdivispz = 0, INCacoplanarity = 0,
            INClabangle = 0, INCbtag = 0, INCtautag = 0, numIncJets = 0, jet2eta1 = 0, 
            jet2eta2 = 0, jet2phi1 = 0, jet2phi2 = 0, jet2PT1 = 0, jet2PT2 = 0, numOIncJets = 0;

    if (parttype == "jet") {
        chosen = select_2_leading(branches["JetOinc"], je);
        if (chosen.first == -1 || chosen.second == -1)
        {
            (*parameters)["INCfoundjets"] = 0;
            // cout << "inclusive should be skipped" << endl;
        }
        else {
            jet2Sel1 = dynamic_cast<Jet*> (branches["JetOinc"]->At(chosen.first));
            jet2Sel2 = dynamic_cast<Jet*> (branches["JetOinc"]->At(chosen.second));
            jet1P4 = jet2Sel1->P4();
            jet2P4 = jet2Sel2->P4();

            jet2eta1 = jet2Sel1->Eta;
            jet2phi1 = jet2Sel1->Phi;
            jet2eta2 = jet2Sel2->Eta;
            jet2phi2 = jet2Sel2->Phi;
            jet2PT1 = jet2Sel1->PT;
            jet2PT2 = jet2Sel2->PT;

            numOIncJets = branches["JetOinc"]->GetEntries();
            numIncJets = branches["JetInc"]->GetEntries();
            (*parameters)["numOIncJets"] = numOIncJets;
            (*parameters)["numIncJets"] = numIncJets;

            sumjet2P4 = jet1P4 + jet2P4;
            diffjet2P4 = CME-sumjet2P4;
            (*parameters)["INCrecoilmass"] = diffjet2P4.M();
            (*parameters)["INCdivism"] = sumjet2P4.M();
            (*parameters)["INCdivispt"] = sumjet2P4.Pt();
            (*parameters)["INCdivispz"] = TMath::Abs(sumjet2P4.Pz());

            //Scaling Test!
            scaledjet1P4 = jet1P4*(MASS_Z/sumjet2P4.M());
            scaledjet2P4 = jet2P4*(MASS_Z/sumjet2P4.M());
            scaledsum2P4 = scaledjet1P4 + scaledjet2P4;
            scaleddiff2P4 = CME-scaledsum2P4;
            (*parameters)["INCscaledrecoilmass"] = scaleddiff2P4.M();
            (*parameters)["INCscaleddivism"] = scaledsum2P4.M();

            (*parameters)["INCeta1"] = jet2eta1; 
            (*parameters)["INCeta2"] = jet2eta2;
            (*parameters)["INCphi1"] = jet2phi1;
            (*parameters)["INCphi2"] = jet2phi2; 
            (*parameters)["INCPT1"] = jet2PT1;
            (*parameters)["INCPT2"] = jet2PT2;
            (*parameters)["INCacoplanarity"] = acoplanarity_angle(jet1P4, jet2P4);
            (*parameters)["INClabangle"] = lab_frame_angle(jet2eta1, jet2phi1, jet2eta2, jet2phi2);

            (*parameters)["INCfoundjets"] = 1;
        }

        if (jet2Sel1 != NULL && jet2Sel2 != NULL){
            (*parameters)["INCbtag"] = jet2Sel1->BTag + jet2Sel2->BTag;
            (*parameters)["INCtautag"] = jet2Sel1->TauTag + jet2Sel2->TauTag; 
        }
    } //branch JetOinc
    ...
    // Parameter filling
    //=======================================================================================
    float numUnweightedEvents = numUnweightedEntries;
    (*parameters)["numUnweightedEvents"] = numUnweightedEvents;

    float eventWeight = weight;
    (*parameters)["eventWeight"] = eventWeight;

    recoilmass = diffparticleP4.M();
    (*parameters)["recoilmass"] = recoilmass;

    float sqrtS = CME.E();
    float recoilmassME = TMath::Sqrt( sqrtS*sqrtS - 2*sqrtS*diffparticleP4.E() + MASS_H*MASS_H );
    (*parameters)["recoilmassME"] = recoilmassME;

    float recoilmassMP = TMath::Sqrt( sqrtS*sqrtS - 2*sqrtS*TMath::Sqrt(MASS_H*MASS_H + diffparticleP4.Px()*diffparticleP4.Px() + diffparticleP4.Py()*diffparticleP4.Py() + diffparticleP4.Pz()*diffparticleP4.Pz())  + MASS_H*MASS_H );
    (*parameters)["recoilmassMP"] = recoilmassMP;
    ...
    if (jetSel1 != NULL && jetSel2 != NULL){
        ...
        tautag = jetSel1->TauTag + jetSel2->TauTag;
        (*parameters)["tautag"] = tautag;
